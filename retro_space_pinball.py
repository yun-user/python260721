...
class RetroPinball:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Retro Space Pinball")
    ...
    def update(self, dt: float, now: float):
        ...
        if not self.game_over and not self.ball_ready:
            ...
            # 발사 레인 위쪽에서 자연스럽게 본 게임판으로 이동
            if self.launching and self.ball_pos.y < 118:
                self.ball_pos.y = 118
                self.ball_vel = Vec2(-410.0, 135.0)
                self.launching = False
